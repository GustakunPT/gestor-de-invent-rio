...
import { 
  ...
  LogOut, Users, Filter, Loader, BarChart2 
} from 'lucide-react';
...
import { 
  ...
  Supplier, PurchaseOrder, AppSettings, User, Customer 
} from './types';
...
const App: React.FC = () => {
  ...
  const [view, setView] = useState<'dashboard' | 'sales_stats' | 'list' | 'history' | 'sales' | 'customers' | 'suppliers' | 'purchases' | 'settings' | 'users'>('dashboard');
  ...
  // Filtra a lista de produtos com base no termo de pesquisa (Search Bar)
  const filteredProducts = useMemo(() => {
    return products.filter(product =>
      // Verifica se o nome contém o termo (case insensitive)
      product.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      // Verifica categoria
      product.category.toLowerCase().includes(searchTerm.toLowerCase()) ||
      // Verifica ID
      product.id.toLowerCase().includes(searchTerm.toLowerCase()) ||
      // Verifica SKU (se existir)
      (product.sku && product.sku.toLowerCase().includes(searchTerm.toLowerCase()))
    );
  }, [products, searchTerm]); // Recalcula se produtos ou termo mudarem
  ...
  const addHistory = async (action: HistoryEntry['action'], productId: string, productName: string, details: string) => {
    const entry: HistoryEntry = {
      id: `HIST-${Date.now()}-${Math.random()}`, // Gera ID único temporário
  ...
  const handleImportHistory = (file: File) => {
      alert("Importação de histórico não implementada nesta versão.");
  };
          ...
          <div className="flex overflow-x-auto space-x-2 md:space-x-4 pb-0 scrollbar-hide">
             {visibleTabs.map((tab) => (
               <button
                 ...
                 className={`flex items-center px-2 py-2 md:px-3 md:py-3 border-b-2 text-xs md:text-sm font-medium whitespace-nowrap transition-colors ${
                   view === tab.id 
                     // Estilo Ativo
